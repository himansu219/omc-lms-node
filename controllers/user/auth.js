...
//     if (!req.body.name) {
//         return next(new AppError("Name field cannot be empty", 500))
//     }
...
//         bcrypt.compare(req.body.oldPassword, checkUserData.password, async(err, ret) => {
//             if (ret) {
...
//                     } else {
//                         const updatePassword = await User.findByIdAndUpdate(checkUserData._id, {
//                             password: await bcrypt.hash(req.body.newPassword, 10),
//                             name: req.body.name,
//                             updated_by: checkUserData._id
//                         });
...
//                 }
//             } else {
//                 return next(new AppError("Invalid Credentials.", 400, { type: "old_password" }));
//             }
//         });
//     }
                ...
                if (req.body.newPassword === req.body.confirmNewPassword) {
                    ...
                    if (req.body.oldPassword === req.body.newPassword) {
                        ...
                        if (updatePassword) {
                            ...
                            const checkUserName = await User.findById({
                                _id: userId,
                                deleted: 0
                            });
                            ...
                            await Activity.create({
                                ipAddress: clientIp,
                                pageDetail: "Change Password",
                                operationType: `Profile MPIN changed by <b>${checkUserName.name}</b> with email address <b>${checkUserName.email}</b>`,
                                userType: userType,
                                created_by: userId
                            });
...
exports.activity = catchAsync(async(req, res, next) => {
    ...
    if (req.body.formdate && req.body.todate) {
        filterObject['createdAt'] = { $gte: new Date(req.body.formdate), $lte: endDate }
    }

    if (req.body.formdate && !req.body.todate) {
        filterObject['createdAt'] = { $gte: new Date(req.body.formdate), $lte: new Date(new Date()) }
    }

    if (req.body.searchdata) {
        filterObject1['operationType'] = { $regex: req.body.searchdata, "$options": "i" }
    }

    if (req.body.searchdata) {
        filterObject2['employeeDetails.name'] = { $regex: req.body.searchdata, "$options": "i" }
    }

    const match_filter = { $or: [filterObject1, filterObject2] };
    let arr = new Array();
    if (req.body.searchdata) {
        ...
        const activityCount = await Activity.aggregate([
            { $sort: { "createdAt": -1 } },
            ...
            bcrypt.compare(req.body.Password, user.password, async(err, ret) => {
                if (ret) {
                    ...
                    if (checkLogin.length == 5) {
                        ...
                        const sendUrl = "https://lms.ntspl.co.in/omc-cms-api/api/v1/user/unblock-account/" + user.id;
                        ...
                        await Activity.create({
                            ipAddress: clientIp,
                            pageDetail: "Login Page",
                            operationType: `Invalid Credentials - <b>${fetchedUser.email}</b> Attempts remaining <b> ${5 - checkLogin.length} </b>`,
                            userType: userType,
                            created_by: fetchedUser._id
                        });
                    }
